...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // DM message content (match-specific)
    const messageForUser = `💖 You’ve been matched with <@${match.id}>!\n`
    ...
    const messageForMatch = `💖 You’ve been matched with <@${member.id}>!\n`
      + `Want to chat? Say hi in DMs!\n\n`
      + `If you're not interested, no worries — you can try again after 24 hours by using the **/match** command in this channel:\n👉 ${MATCH_CHANNEL_LINK}`;

    // DM both users correctly
    try {
      await member.send({ content: messageForUser, embeds: [embed] });
    } catch (err) {
      console.warn(`❌ Could not DM ${member.user.tag}`);
    }

    try {
      await match.send({ content: messageForMatch, embeds: [embed] });
    } catch (err) {
